...
export default function UserLinks({ username }: { username: string }) {
  const [links, setLinks] = useState<{ url: string; platform: string }[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const userLinks = localStorage.getItem(username);
    if (userLinks) {
  ...
  return (
    <>
      ...
      <motion.h1
        ...
        className="text-3xl font-bold text-center text-green-800 dark:text-green-200 mb-12"
      >
        @{username}
      </motion.h1>
      ...
      <div className="relative">
        ...
        <motion.div
          ...
          className="absolute left-1/2 transform -translate-x-1/2 w-1 bg-green-600 dark:bg-green-400"
          ...
          {links.map((link, index) => (
            ...
            >
              ...
              <motion.div
                ...
                className={`absolute top-1/2 h-1 bg-green-600 dark:bg-green-400 ${
